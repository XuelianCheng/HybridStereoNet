...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(x, 'b d h w (p1 p2 p3 c)-> b (d p1) (h p2) (w p3) c', p1=self.dim_scale, p2=self.dim_scale, p3=self.dim_scale, c=C // 8)
        x = self.norm(x)
...
class FinalPatchExpand_X2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, D, H, W, C)
        x = rearrange(x, 'b d h w (p1 p2 p3 c)-> b (d p1) (h p2) (w p3) c', p1=self.dim_scale, p2=self.dim_scale, p3=self.dim_scale,
...
class PatchEmbed3D(nn.Module):
    ...
    def __init__(self, img_size=(128, 128, 128), patch_size=(4, 4, 4), in_chans=3, embed_dim=96, norm_layer=None):
        ...
        self.patches_resolution = patches_resolution #[32, 32, 32]

        # self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=patch_size, stride=2, padding=1)
...
class MatchingNet(nn.Module):
    ...
    def __init__(self, pretrained=None,
        ...
        print(
            "MatchingNet depths:{}; depths_decoder:{}; drop_path_rate:{}; num_classes:{}; embed_dims:{}; num_heads:{}; window:{}".format(
                depths, depths_decoder, drop_path_rate, num_classes, embed_dim, num_heads, window_size))
        ...
        patches_resolution = self.patch_embed.patches_resolution #(32, 32, 32)
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up = FinalPatchExpand_X2(input_resolution=(
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                x = torch.cat([x, x_downsample[self.num_layers-inx-1]], 1)
                B, C, D, H, W = x.shape
                x = x.flatten(2).transpose(1, 2)
    ...
    # Dencoder and Skip connection
    def forward_up_features_1(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = layer_up(x)
            else:
    ...
    def forward(self, x):
        # torch.Size([1, 96, 48, 224, 336])
        # pdb.set_trace()
        x, x_downsample = self.forward_features(x) 
        # pdb.set_trace()
        # (Pdb) x_downsample[0].shape
        # torch.Size([1, 96, 24, 112, 168])
        # (Pdb) x_downsample[1].shape
        # torch.Size([1, 192, 12, 56, 84])
        # (Pdb) x_downsample[2].shape
        # torch.Size([1, 384, 6, 28, 42])
        # (Pdb) x_downsample[3].shape
        # torch.Size([1, 768, 3, 14, 21])

        x = self.forward_up_features(x, x_downsample)
        # pdb.set_trace()
        # torch.Size([1, 24, 112, 168, 96])

        x = self.up_x2(x)
        # torch.Size([1, 1, 48, 224, 336])
